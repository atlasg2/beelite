...
export default async function PermitsPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const buildHref = (overrides: Record<string, string | number | string[] | undefined>) => {
    ...
    const set = { q, work, cls, view: view === "active" ? "" : view, page: page > 1 ? page : "", ...overrides };
  ...
  const hasFilters = !!(q || work || cls || codes.length || view !== "active");
                ...
                <tr key={r.id} data-lead={r.leadStatus}>
                  <td>
                    <div className="lead-actions">
                      {r.leadStatus === "saved" ? (
                        <span className="lead-badge saved">⚑ Saved</span>
                      ) : (
                      ...
                      {r.leadStatus === "dismissed" ? (
                        <span className="lead-badge dismissed">Hidden</span>
                      ) : (
                        <form action={setLeadStatus}>
                          <input type="hidden" name="id" value={r.id} />
                      ...
                      {r.leadStatus !== "new" && (
                        <form action={setLeadStatus}>
                          ...
                          <button type="submit" className="btn-mini reset" title="Undo — back to untriaged">↺</button>
                        </form>
                      )}
                    </div>
                  </td>
                  <td className="mono">{r.permitNum}</td>
